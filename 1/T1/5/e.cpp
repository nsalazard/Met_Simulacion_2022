...
void Grafica(void){
  ...
  cout<<"set out 'Yprom.pdf'"<<endl;
	cout<<"set title 'Altura promedio'"<<endl;
  cout<<"set ylabel 'Altura promedio'"<<endl;
	cout<<"set xlabel 'Tiempo'"<<endl;
	cout<<"set autoscale"<<endl;
	cout<<"set key"<<endl;
	cout<<"set font ',7'"<<endl;
	cout<<"plot 'Yprom.txt' u 1:2 w lp t 'Datos'"<<endl;
  ...
  for(ix=0;ix<Nx;ix++)
    ...
    if(tdibujo>tcuadro){
		//Imprimir promedio
			double Yprom = 0.0;
      for(i=0;i<N;i++){
				Yprom += Molecula[i].Gety();
				}
			Yprom = Yprom/N;
      HistoY << ti << "\t" << Yprom << endl;
      tdibujo=0;
			ti+=1;
    }
    //--- Muevase por PEFRL ---
